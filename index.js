  ...
  this.socket.on('connect', function() {
    ...
    this.socket.once('data', function(response) {
      if (!response || !response.length) {
        error = new Error('Error sending passport, cause response is empty');
      ...
      if (response.toString('ascii').indexOf('OK') === 0) {
        ...
        if (response.length > 2) {
          transport.handleResponse(response.slice(2));
        }
      }
      else {
    ...
    this._send(helloBuf, function(error) {
      if (error) {
...
function bindHandleData(transport) {
  transport.socket.on('data', function(data) {
    transport.debug && console.log('[%s] received: ' + JSON.stringify(data) + ', string:' + data.toString('ascii'), transport);
    transport.handleResponse(data);
  }.bind(transport));
}
...
WiFiIO.prototype.readIO = function(ioNumber, callback) {
  this._send(this._buildPacket(0x13, parseInt(ioNumber)), function() {
    if (error)
      return callback && callback(error);
    this.once('input.' + ioNumber.toString(), function(response) {
...
WiFiIO.prototype.readAllIO = function(callback) {
  this._send(this._buildPacket(0x14), function() {
    if (error)
      return callback && callback(error);
    this.once('input.all', function(response) {
      callback && callback(null, response);
    });
  }.bind(this), callback);
}
...
function handleQueue(transport) {
  ...
  if (item !== null && item !== undefined) {
    ...
    transport.once('response', function(response) {
...
WiFiIO.prototype._send = function(packet, callback) {
  if (packet === '' || packet === null)
    return callback && callback(new Error('Empty command parameter'));
  this.queue.push({id: nextId(), packet: packet, callback: callback});
...
function buildCallbackInner(_item, _transport) {
  return function(item, transport) {
    return function(error, response) {
      transport.debug && console.log('usriot: callbackInner(err[%s], response[%s]) for item[%s]', JSON.stringify(error), JSON.stringify(response), JSON.stringify(item));
      stopSendTimeout();
